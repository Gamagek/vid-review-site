...
await patch("src/index.js", [
  ...
  ['  if (!CATEGORIES[category].includes(subcategory)) throw new AppError(400, "Select a valid subcategory");', '  if (category !== "Other" && !CATEGORIES[category].includes(subcategory)) throw new AppError(400, "Select a valid subcategory");\n  if (category === "Other" && (!subcategory || subcategory.length > 80)) throw new AppError(400, "Enter a valid custom subcategory");'],
]);

await patch("public/admin.html", [
  ['<label><span>Subcategory</span><select id="admin-subcategory" name="subcategory" required disabled><option value="">Choose subcategory</option></select></label>', '<label><span>Subcategory</span><select id="admin-subcategory" name="subcategory" required disabled><option value="">Choose subcategory</option></select><input id="admin-other-subcategory" type="text" maxlength="80" placeholder="Type your subcategory" autocomplete="off" hidden></label>'],
]);

await patch("public/admin.js", [
  ['  subcategory: document.querySelector("#admin-subcategory"),\n  title:', '  subcategory: document.querySelector("#admin-subcategory"),\n  otherSubcategory: document.querySelector("#admin-other-subcategory"),\n  title:'],
  ['  ui.category.addEventListener("change", () => fillSubcategories(ui.category.value));', '  ui.category.addEventListener("change", () => fillSubcategories(ui.category.value));\n  ui.otherSubcategory.addEventListener("input", syncOtherSubcategory);'],
  ...
  ['function setSourceMode(mode) {', 'function syncOtherSubcategory() {\n  if (ui.category.value !== "Other") return;\n  const value = ui.otherSubcategory.value.trim();\n  const existing = [...ui.subcategory.options].find((option) => option.dataset.custom === "1");\n  if (existing) existing.remove();\n  const option = document.createElement("option");\n  option.value = value;\n  option.textContent = value || "Type your subcategory";\n  option.dataset.custom = "1";\n  option.hidden = !value;\n  ui.subcategory.append(option);\n  ui.subcategory.value = value;\n}\n\nfunction setSourceMode(mode) {'],
  ['    subcategory: ui.subcategory.value,', '    subcategory: ui.category.value === "Other" ? ui.otherSubcategory.value.trim() : ui.subcategory.value,'],
]);

await writeFile("scripts/patch-other-category.mjs", "// one-time patch consumed by deploy workflow\n");
